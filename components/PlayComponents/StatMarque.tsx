          ...
          <View style={{ flexDirection: 'row' }}>
            ...
            <Text
              style={[
                styles.marqueeText,
                styles.textShadowMarquee,
                (round.totalFIR / Object.values(round.holes).filter(hole => hole.hole.par !== 3).length) * 100 >= Number(firGoal) / 18
                  ? styles.goodText
                  : styles.badText,
              ]}
            >
              FIR: {formatPercentage(round.totalFIR / Object.values(round.holes).filter(hole => hole.hole.par !== 3).length)}%
            </Text>
